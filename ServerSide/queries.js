...
const getRecipeByName = (request, response) => {
  ...
  pool.query("SELECT * FROM recipes WHERE (name ~ $1) OR (name ~ LOWER($1)", [name], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
...
const getRecipeByIngredient = (request, response) => {
  ...
  pool.query("SELECT * FROM recipes WHERE EXISTS (SELECT * FROM unnest(ingredients) elem WHERE elem ~* $1)", [ingredient], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
...
const getRecipeByTag = (request, response) => {
  ...
  pool.query("SELECT * FROM recipes WHERE EXISTS (SELECT * FROM unnest(tags) elem WHERE elem ~* $1)", [tag], (error, results) => {
    if (error) {
      throw error
    }
    response.status(200).json(results.rows)
